...
#D6_vs_D3===========================
{
  #plot SupFigure 2 A middle
  {
    WT_KO=Nine_Square(WT,KO,myXlabel="log2(D6/D3) in WT",myYlabel="log2(D6/D3) in KO",org="mmu")
    SupFigure2A_Middle=WT_KO$nineSquare
    SupFigure2A_Middle
  }
  
  #plot SupFigure 2 B middle and SupFigure 3 B left-up
  {
    ...
    Group_FI=Nine_Square(RE1,KO1,myXlabel="log2(D6/D3) in KO+WT",myYlabel="log2(D6/D3) in KO",org="mmu")
    SupFigure2B_Middle=Group_FI$nineSquare
    #============================================================
    myGroup=Group_FI$data[Group_FI$data$group=="I"|Group_FI$data$group=="F",]
    genes=row.names(myGroup)
    myAll=All[All$geneSymbol %in% genes,]
    myTitle="D6 vs D3 Dynamic of Group F&I"
    
    SupFigure3B_LeftUp=DifferentialCourse(myAll,myTitle)
    SupFigure3B_LeftUp
  }
  
  #plot SupFigure 3 B left-down
  {
    suppressPackageStartupMessages(library(org.Mm.eg.db))
    OrgDb=org.Mm.eg.db
    genes=WT$exp[,c(1,5)]
    ...
    SupFigure3B_LeftDown=EN$GOBPenrich_plot
    SupFigure3B_LeftDown
  }
  
  #plot SupFigure 3 B right
  {
    myGroup=WT_KO$data[WT_KO$data$group=="I"|WT_KO$data$group=="F",]
    genes=row.names(myGroup)
    WT1=WT
    ...
    dd2=dd2[1:50,]
    SupFigure3B_Right=pheatmap(dd2,cluster_cols = F,fontsize = 7,main="Top 50 Representative Rescured Genes")
    SupFigure3B_Right
  }
  
  #plot SupFigure 2 C middle and SupFigure 3 E left-up
  {
    ...
    KO1=KO
    KO1$exp=KO1$exp[KO1$exp$geneSymbol %in%genes, ]
    Group_AD=Nine_Square(RE1,KO1,myXlabel="log2(D6/D3) in KO+WT",myYlabel="log2(D6/D3) in KO",org="mmu")
    SupFigure2C_Middle=Group_AD$nineSquare
    
    myGroup=Group_AD$data[Group_AD$data$group=="A"|Group_AD$data$group=="D",]
    genes=row.names(myGroup)
    myAll=All[All$geneSymbol %in% genes,]
    myTitle="D6 vs D3 Dynamic of Group A&D"
    
    SupFigure3E_LeftUP=DifferentialCourse(myAll,myTitle)
    SupFigure3E_LeftUP 
  }
  
  #plot SupFigure 3 E left-down
  {
    suppressPackageStartupMessages(library(org.Mm.eg.db))
    OrgDb=org.Mm.eg.db
    genes=WT$exp[,c(1,5)]
    ...
    genes=genes$ENTREZID
    EN=enrichment_analysis(geneList$geneList,genes,plotTitle=myTitle,org="mmu",type=c("GO"),GOcatagory=c("BP"),output=T)
    SupFigure3E_LeftDown=EN$GOBPenrich_plot
    SupFigure3E_LeftDown
  }
  
  #plot SupFigure 3 E right
  {
    myGroup=WT_KO$data[WT_KO$data$group=="A"|WT_KO$data$group=="D",]
    genes=row.names(myGroup)
    WT1=WT
    ...
    #dd2=dd2[1:50,]
    SupFigure3E_Right=pheatmap(dd2,cluster_cols = F,fontsize = 7,main="Top 40 Representative Rescured Genes")
    SupFigure3E_Right
  }
}
