...
{
  load("WT_D12_vs_D6.RData")
  WT=diffGeneEnrich
  load("NonoKO_D12_vs_D6.RData")
  KO=diffGeneEnrich
  load("Rescue_D12_vs_D6.RData")
  RE=diffGeneEnrich
  rm(diffGeneEnrich)
}
#D12_vs_D6===========================
{
  #plot SupFigure 2 A right
  {
    WT_KO=Nine_Square(WT,KO,myXlabel="log2(D12/D6) in WT",myYlabel="log2(D12/D6) in KO",org="mmu")
    SupFigure2A_Right=WT_KO$nineSquare
    SupFigure2A_Right
  }
  
  #plot SupFigure 2 B right and SupFigure 3 C left-up
  {
    ...
    Group_FI=Nine_Square(RE1,KO1,myXlabel="log2(D12/D6) in KO+WT",myYlabel="log2(D12/D6) in KO",org="mmu")
    SupFigure2B_Right=Group_FI$nineSquare
    SupFigure2B_Right
    #============================================================
    myGroup=Group_FI$data[Group_FI$data$group=="I"|Group_FI$data$group=="F",]
    genes=row.names(myGroup)
    myAll=All[All$geneSymbol %in% genes,]
    myTitle="D12 vs D6 Dynamic of Group F&I"
    
    SupFigure3C_LeftUp=DifferentialCourse(myAll,myTitle)
    SupFigure3C_LeftUp
  }
  
  #plot SupFigure 3 C left-down
  {
    suppressPackageStartupMessages(library(org.Mm.eg.db))
    OrgDb=org.Mm.eg.db
    genes=WT$exp[,c(1,5)]
    ...
    SupFigure3C_LeftDown=EN$GOBPenrich_plot
    SupFigure3C_LeftDown
  }
  
  #plot SupFigure 3 C right
  {
    myGroup=WT_KO$data[WT_KO$data$group=="I"|WT_KO$data$group=="F",]
    genes=row.names(myGroup)
    WT1=WT
    ...
    dd2=dd2[1:50,]
    SupFigure3C_Right=pheatmap(dd2,cluster_cols = F,fontsize = 7,main="Top 50 Representative Rescured Genes")
    SupFigure3C_Right
  }
  
  #plot SupFigure 2 C right and SupFigure 3 F left-up
  {
    ...
    KO1=KO
    KO1$exp=KO1$exp[KO1$exp$geneSymbol %in%genes, ]
    Group_AD=Nine_Square(RE1,KO1,myXlabel="log2(D12/D6) in KO+WT",myYlabel="log2(D12/D6) in KO",org="mmu")
    SupFigure2C_Right=Group_AD$nineSquare
    
    myGroup=Group_AD$data[Group_AD$data$group=="A"|Group_AD$data$group=="D",]
    genes=row.names(myGroup)
    myAll=All[All$geneSymbol %in% genes,]
    myTitle="D12 vs D6 Dynamic of Group A&D"
    
    SupFigure3F_LeftUP=DifferentialCourse(myAll,myTitle)
    SupFigure3F_LeftUP 
  }
  
  #plot SupFigure 3 F left-down
  {
    suppressPackageStartupMessages(library(org.Mm.eg.db))
    OrgDb=org.Mm.eg.db
    genes=WT$exp[,c(1,5)]
    ...
    genes=genes$ENTREZID
    EN=enrichment_analysis(geneList$geneList,genes,plotTitle=myTitle,org="mmu",type=c("GO"),GOcatagory=c("BP"),output=T)
    SupFigure3F_LeftDown=EN$GOBPenrich_plot
    SupFigure3F_LeftDown
  }
  
  #plot SupFigure 3 F right
  {
    myGroup=WT_KO$data[WT_KO$data$group=="A"|WT_KO$data$group=="D",]
    genes=row.names(myGroup)
    WT1=WT
    ...
    #dd2=dd2[1:50,]
    SupFigure3F_Right=pheatmap(dd2,cluster_cols = F,fontsize = 7,main="Top 49 Representative Rescured Genes")
    SupFigure3F_Right
  }
}
